...
这个项目不是“让大模型直接控制机器人”，而是为 AutoLife S2 建立一条可复现、
可检查、失败时安全停止的视觉抓取链。当前目标很具体：机器人保持底盘不动，使用
头部 RGB-D 相机识别桌面上的瓶子，计算瓶子与机械臂 TCP 的三维关系，为左臂生成
侧向抓取候选，并由 MoveIt 验证可达性、自碰撞、桌面碰撞和轨迹约束。

现场原始系统存在四个关键落差：

1. RGB、深度、mask 和相机内参来自实时共享内存，若分开读取就可能错帧。
2. GraspNet 的通用 6D 候选不理解“竖直瓶子应侧向夹持”，候选可能全部被方向过滤。
3. MoveIt 使用双 7 轴机器人模型，而厂商接口要求腰腿 4 + 双臂 14 的 18 值轨迹，
   且没有标准 `FollowJointTrajectory` 执行入口。
4. 桌面、相机外参、TCP 和夹爪映射必须来自现场测量，不能由模型猜测或写死。

因此系统把感知、候选生成、运动规划和硬件授权分成独立边界；任何一层不确定，
本次抓取就失败关闭，而不是跳过检查继续运动。

## 组件职责

| 组件 | 负责 | 不负责 |
| --- | --- | --- |
| YOLO11 + RGB-D bridge | 同帧实例、深度、mask、内参 | 生成关节命令 |
| GraspNet | 目标点云上的抓取分数、宽度与初始 6D 候选 | 判断整臂可达性 |
| 瓶子侧抓适配层 | TCP 可达域、水平闭合轴、自然腕姿等价方向 | 放宽碰撞或角度阈值 |
| MoveIt | IK、关节限位、自碰撞、桌面碰撞、路径和时间参数化 | 绕过现场验收 |
| MotionExecutor | 把已验证轨迹适配为厂商 18 值格式并执行最终安全门 | 自行选择目标或修改安全配置 |
| OpenClaw skills | 自然语言任务入口、调用固定工具、整理诊断 | 实时控制环或直接发布 ROS 运动话题 |

`visual-navigation` 仅保留相机理解和导航 dry-run 能力，不参与当前抓取闭环，也不移动
底盘。当前实机工作只针对机械臂。

## 一次瓶子抓取如何处理

1. 从共享内存取得同一时刻的彩色图和对齐深度，绑定实例 mask、内参、时间戳与哈希。
2. 把目标像素与深度转换到机器人基座坐标，并读取当前左右 TCP 位姿。
3. GraspNet 在目标点云上生成候选并执行局部场景点云碰撞过滤。
4. 对瓶子额外生成侧抓候选：接近轴由当前 TCP 指向精确 RGB-D 瓶子中心，闭合轴与
   重力方向垂直，并选择接近当前腕姿的平行夹爪等价方向。
5. 推理结束后用同一 track 的新帧检查目标是否移动；移动则丢弃全部候选。
6. MoveIt 从当前关节反馈依次验证 pregrasp、grasp 和 lift；任一 IK、碰撞、路径或
   时间参数化失败都拒绝本次任务。
7. 默认只返回 dry-run 结果。真实执行还需要现场标定、实体急停测试、签名验收、
   配置解锁和精确操作员确认令牌。
